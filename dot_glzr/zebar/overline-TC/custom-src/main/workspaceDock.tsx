...
const expandedStorageKey = 'overline-TC.workspaceDock.expanded';
...
export function WorkspaceDock({ glazewm }: WorkspaceDockProps) {
  ...
  const expandedWidth = Math.max(0, Math.min(windows.length * 32 - 4, 352));
  const toggleLabel = `${expanded ? '收合' : '展開'}目前工作區的 ${windows.length} 個程式視窗`;
  ...
  return (
    ...
    >
      <button
        type="button"
        className="flex h-7 min-w-7 shrink-0 items-center justify-center border border-transparent bg-button px-1.5 text-[10px] font-semibold text-icon transition-colors hover:bg-button-hover focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
        title={toggleLabel}
        aria-label={toggleLabel}
        aria-expanded={expanded}
      ...
      >
        ...
        <span className="ml-1">{windows.length}</span>
      </button>
      {windows.length > 0 && (
        <motion.div
          initial={false}
