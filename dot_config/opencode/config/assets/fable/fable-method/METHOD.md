...
**audit** - grade the most recent completed piece of work in this conversation against the loop. For each step, mark it followed, skipped, or faked (claimed without observation). For every skip or fake, name the concrete risk it created; use `Read` on `~/.config/opencode/config/assets/fable/fable-method/references/failure-modes.md` to map symptoms to steps. Deliver a short table plus the single highest-value fix, and apply that fix only if the user asks.
