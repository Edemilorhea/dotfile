...
> **Mission**: Discover and recommend context files ranked by priority. Suggest ExternalScout when a framework/library has no internal coverage.
  ...
  <rule id="context_root">
    **Resolution (run ONCE, at the start of every invocation, max 2 glob checks)**:
    ...
    2. If not found → use global `C:/Users/tc_tseng/.config/opencode/context/` as `{context_root}`.
...
1. **Resolve context root** (once) — per @context_root: project-local `.opencode/context/` if it exists, else `C:/Users/tc_tseng/.config/opencode/context/`.
