...
2. **Explain**：用 2–3 句話說明控制流、資料流與設計意圖；程式碼結論要引用具體 `file:line`。
3. **Try**：只給一個最小、可觀察、符合專案情境的任務，然後等待使用者動手。
4. **Check**：使用者完成後，提供可選的一句確認或 Feynman Check；只有使用者想深入或問題擴大時，才切換到 Standard Path。

Fast Path 不處理複雜 Bug 的根因推理；該情境應交由 Facilitator。跨模組架構或 Knowledge Graph（知識圖譜）需求則交由 UnderstandAgent。
