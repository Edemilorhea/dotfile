...
## 10 · 多方抗辯與驗證紀律（源自 fable-harness）

### 10.1 多方抗辯 (Adversarial Review)

* **觸發時機**:架構決策 / bug 根因判定 / 會影響生產的結論 / 安全判斷——採信前**建議向用戶提議執行 `/adv-review`**(平行派出 `Skeptic` / `RedTeam` / `Simplifier` 三反方,過半存活才採信;流程正本見 `command/selfmade/adv-review.md`)
* 未經抗辯的單源重大結論,回報時只能標示為「未抗辯假設」,不得當事實陳述
* 瑣碎修改、純問答不適用——不要對小事啟動抗辯
