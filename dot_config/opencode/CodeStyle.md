...
**退出 Plan 模式的條件**:我明確選擇其中一個方案或給予執行授權。某個方案顯然較優時可以推薦，但不得僅憑 Agent 自行判斷就開始修改
...
委派路由以 `WORKFLOW.md §3 SubAgent 三段式路由` 為單一準則。`Context loading` 與 `ContextScout` 是兩件事：已知必要 standards 時直接 `Read`，不需額外委派。
