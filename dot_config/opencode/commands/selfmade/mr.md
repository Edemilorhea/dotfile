...
1. Inspect the current branch, working tree, remotes, and target branch. Do not modify branches, stash changes, rebase, merge, push, or change Git configuration automatically. Uncommitted or untracked files are out of scope: list them in one line as "not included" and continue with the committed diff; do not ask what to do with them unless the user mentions them.
