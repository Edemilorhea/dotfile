...
After handling the steering instruction, resume the original active task from its latest valid checkpoint and carry it to completion, including any verification it already requires. Do not replace, cancel, or silently abandon the original task unless the user explicitly instructs you to do so.
