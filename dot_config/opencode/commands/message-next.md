...
Treat the instruction below as a separate follow-up that depends on the active task. Record it as a pending Todo now, but do not let it change the active task's scope or acceptance criteria. Finish the active task first, including any verification it already requires. Then promote this follow-up to `in_progress` and execute it in the same session unless it is blocked.
